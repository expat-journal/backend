...
/**
 * @api {post} /posts/search   Search posts
 * @apiVersion 1.0.0
 * @apiName SearchPosts
 * @apiGroup Posts
 *
 * @apiHeader {String} authorization    The token given to the user at login.
 *
 * @apiParam {String} query             String to search for.
 *
 * @apiExample Search example:
 * const instance = axios.create({
        baseURL: 'http://localhost:3200',
        timeout: 1000,
        headers: {
...
postsRouter.post( "/search", ( req, res ) => {
    ...
    if ( !query ) {
        return res.status( 400 ).
            json( {
                message: "You must include string to search for.",
                status:  400
            } );
    }
    
    getAllPosts().then( async results => {
